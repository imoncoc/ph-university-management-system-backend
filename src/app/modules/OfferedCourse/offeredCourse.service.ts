...
import { AcademicFaculty } from '../academicFaculty/academicFaculty.module';
...
const createOfferedCourseIntoDB = async (payload: TOfferedCourse) => {
  ...
  // Check id the department is belong to the faculty
  ...
  const isSameOfferedCourseExistsWithRegisteredSemesterWithSameSection =
    await OfferedCourse.findOne({ semesterRegistration, course, section });

  if (isSameOfferedCourseExistsWithRegisteredSemesterWithSameSection) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
  ...
  if (hasTimeConflict(assignedSchedule, newSchedule)) {
    throw new AppError(
      httpStatus.CONFLICT,
      `This faculty is not available at that! Choose other time or day`,
    );
  }
